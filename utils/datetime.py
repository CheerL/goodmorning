...
class Tz:
    ...
    @classmethod
    def get_tz(cls, num=None):
        return TZ_DICT[num or cls.tz_num]
...
def ts2dt(ts=0, tz=None):
    if ts:
        dt = datetime.datetime.fromtimestamp(ts)
    else:
        dt = datetime.datetime.now()

    return dt.astimezone(tz=Tz.get_tz(tz))

def ts2time(ts=0, fmt='%Y-%m-%d %H:%M:%S', tz=None):
    return ts2dt(ts, tz).strftime(fmt)


def ts2date(ts=0, tz=None):
    return ts2time(ts, '%Y-%m-%d', tz)

def ts2level_hour(ts=0, level_ts=86400, tz=None):
    if ts == 0:
        ts = time.time()
    return ts2time(ts // level_ts * level_ts, '%Y-%m-%d-%H', tz)